...
const KEYFRAMES = `
  ...
  @keyframes sb-idle-pulse {
    ...
    50%       { box-shadow: 0 0 5px var(--tier-glow); }
  }
`;
...
type AnyParticle = SnowParticle | BubbleParticle | RainParticle | OrbitParticle | SparkParticle | FlameParticle | ConfettiParticle;
...
function makeParticles(type: ParticleType, _color: string): AnyParticle[] {
  ...
  return Array.from({ length: n }, (_, i): AnyParticle => {
    const base = { id: i, delay: `${rnd(0,.45).toFixed(2)}s`, dur: `${rnd(.6,1.4).toFixed(2)}s`, size: 0 };
    ...
    return { ...base, size:rnd(3,6), color:CC[i%CC.length], cx:`${(Math.cos(a)*d).toFixed(1)}px`, cy:`${(Math.sin(a)*d).toFixed(1)}px`, cr:`${rnd(-270,270).toFixed(0)}deg` } as ConfettiParticle;
  });
}

function Particle({ type, p, color }: { type: ParticleType; p: AnyParticle; color: string }) {
  const c = p.color ?? color;
  const base: CSSProperties = { position:"absolute", borderRadius:type==="rain"?"2px":"50%", pointerEvents:"none", background:c, boxShadow:type==="confetti"?"none":`0 0 ${p.size*2}px ${c}`, animationFillMode:"forwards", width:p.size, height:p.size, opacity:0 };
  if (type==="snow")     { const sp=p as SnowParticle;     return <div style={{...base,left:`${sp.x}%`,top:`${sp.y}%`,"--dx":sp.dx,animation:`sb-snow ${p.dur} ease-out ${p.delay} forwards`} as CSSProperties}/>; }
  if (type==="bubble")   { const bp=p as BubbleParticle;   return <div style={{...base,left:`${bp.x}%`,top:`${bp.y}%`,animation:`sb-bubble ${p.dur} ease-out ${p.delay} forwards`}}/>; }
  if (type==="rain")     { const rp=p as RainParticle;     return <div style={{...base,height:rp.h,borderRadius:"1px",left:`${rp.x}%`,top:`${rp.y}%`,animation:`sb-rain ${p.dur} linear ${p.delay} forwards`}}/>; }
  if (type==="orbit")    { const op=p as OrbitParticle;    return <div style={{...base,left:"50%",top:"50%",marginLeft:-p.size/2,marginTop:-p.size/2,"--start-deg":op.sd,"--r":op.r,animation:`sb-orbit ${p.dur} linear ${p.delay} forwards`,transformOrigin:"0 0"} as CSSProperties}/>; }
  if (type==="spark")    { const sp=p as SparkParticle;    return <div style={{...base,left:"50%",top:"50%","--sx":sp.sx,"--sy":sp.sy,animation:`sb-spark ${p.dur} ease-out ${p.delay} forwards`} as CSSProperties}/>; }
  if (type==="flame")    { const fp=p as FlameParticle;    return <div style={{...base,height:p.size*1.4,borderRadius:"50% 50% 40% 40%",left:`${fp.x}%`,top:`${fp.y}%`,"--fx":fp.fx,animation:`sb-flame ${p.dur} ease-out ${p.delay} forwards`} as CSSProperties}/>; }
  const cp=p as ConfettiParticle; return <div style={{...base,borderRadius:"2px",left:"50%",top:"50%","--cx":cp.cx,"--cy":cp.cy,"--cr":cp.cr,animation:`sb-confetti ${p.dur} ease-out ${p.delay} forwards`} as CSSProperties}/>;
}
...
export default function StarBadge({ shieldsUrl, repoLink, githubToken }: StarBadgeProps) {
  ...
  const handleEnter = useCallback(() => { if (!tier) return; setHovered(true); setParticles(makeParticles(tier.particleType, tier.color)); }, [tier]);
  ...
  if (error || starCount === null || !tier) {
    return <img src={shieldsUrl} alt="GitHub stars" className="h-5 align-middle" />;
  }
  ...
  // ── 默认状态色值（等级色 + 透明度叠白底）
  // border:  33% → 淡色微光轮廓，清晰但不刺眼
  // bg:       6% → 几乎看不出颜色，只是隐约的底色
  // icon/text:73%/80% → 有辨识度，不过曝
  ...
  const wrapperStyle: CSSProperties = {
    ...
    padding: "3px 10px",
    borderRadius: 999,
    background: hovered ? "rgba(0,0,0,0.55)" : `${hex}10`,
    border: `1px solid ${hovered ? hex : `${hex}55`}`,
    boxShadow: hovered ? `0 0 10px ${tier.glow}, 0 0 20px ${tier.glow}` : "none",
    // 非 hover 时有一个极轻微的呼吸感
    animation: hovered ? "none" : `sb-idle-pulse 3.5s ease-in-out infinite`,
    "--tier-glow": tier.glow,
    cursor: repoLink ? "pointer" : "default",
    textDecoration: "none",
    transition: "background 0.28s, border-color 0.28s, box-shadow 0.28s, transform 0.28s cubic-bezier(0.34,1.56,0.64,1)",
  ...
  return (
    <Wrapper {...linkProps} onMouseEnter={handleEnter} onMouseLeave={handleLeave} style={wrapperStyle}>
      {isGold && hovered && (
        <div style={{ position:"absolute", inset:0, borderRadius:999, pointerEvents:"none", background:"linear-gradient(90deg,transparent 0%,rgba(251,191,36,0.35) 50%,transparent 100%)", backgroundSize:"200% 100%", animation:"sb-shimmer 1.1s linear infinite" }}/>
      )}
      ...
      <svg
        ...
        fill={hovered ? hex : `${hex}bb`}
        style={{ transition:"fill 0.25s", filter: hovered ? `drop-shadow(0 0 3px ${hex})` : "none", flexShrink:0 }}
      >
        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
      </svg>
